...
permuteResult <- generateNull(n_up = n_up, n_down = n_down,
                              rankData = rankData[,1:5],B = 1000,
                              ncores = ncores,
                              seed = 1,
                              useBPPARAM = NULL)
...
2. CCLE dataset
  ...
  `scoredf_ccle_epi` & `scoredf_ccle_mes` are two pre-computed singscores (data frames) of The Cancer Cell Line Encyclopaedia (CCLE) breast cancer cell line RNA-seq dataset [@barretina2012cancer]. The CCLE dataset was normalised by TPM and can be downloaded from https://www.synapse.org/#!Synapse:syn5612998 It was scored against the undirected epithelial and mesenchymal gene signatures respectively. The gene signatures were obtained from [@Ep-MES] and can be found in the 'Table S1B. Generic EMT signature for cell line' in the supplementary files.  
  The gene expression ma

3. TCGA cancer samples dataset

   `scoredf_tcga_epi`&`scoredf_tcga_mes` are two pre-computed singscores (data frames) of The Cancer Genome Atlas (TCGA) breast cancer RNA-seq data (RSEM normalised) [@TCGAcite] against the mesenchymal and epithelial gene signatures respectively. The gene signatures were obtained from [@Ep-MES] and can be found in the 'Table S1A. Generic EMT signature for tumour'. The TCGA dataset was downloaded from The UCSC Cancer Genomics Browser in February 2016  (https://genome-cancer.ucsc.edu)
