...
---
This vignette demonstrates how to use SINCORE to score a gene expression dataset against a gene set on a single-sample level. It contains 6 parts:

- Introduction of SINCORE R package
- Installing SINCORE
- Preparing Dataset
...
## Introduction 
SINCORE implements a simple single-sample scoring method which does not rely on other the samples in gene expression data sets and can deal with single-sample input. It provides stable scores which are also less likely to be affected by unwanted variations across samples. CITATION
It also provides various visulisation functions to further explore the analysis result.

## Install SINCORE R package
SINCORE is currently host on github and can be easily installed using 'install_github()' function provided by 'devtools' CITATION.
```{r installation, eval=FALSE}
library(devtools)
install_github("https://github.com/Rachael-rq/SINCORE")
...
*Maybe some comment on the plot
...
```{r pvalue, fig.height = 6, fig.width = 10}
...
plot_null(permuResult,scoredf,pvals,sampleLabel = names(pvals)[1:5])
